...
def print_label(template_path: str | Path, placeholders: dict[str, str]):
    ...
    """
    # --- DEBUG: show inputs ---
    print(f"[DEBUG] print_label(): template_path={template_path}")
    print(f"[DEBUG] placeholders keys: {list(placeholders.keys())}")
    ...
    # --- DEBUG: show a snippet of the filled XML ---
    snippet = xml_filled[:200].replace("\n", " ")
    print(f"[DEBUG] First 200 chars of filled XML: {snippet}")
        ...
        if tag == "table":
            ...
            if cells is not None:
                ...
                for idx, cell in enumerate(texts):
                    ...
                    # DEBUG: table cell info
                    print(f"[DEBUG] table cell #{idx}: content='{content}' row={r} col={c} → tx={tx:.1f}, ty={ty:.1f}")
                    ...
                    if cell.find("fontStyle") is not None:
                        text_args["fontStyle"] = cell.findtext("fontStyle", "0x00")
                    if cell.find("verticalAlignment") is not None:
                        text_args["verticalAlignment"] = int(cell.findtext("verticalAlignment", 0))
                    if cell.find("horizontalAlignment") is not None:
                        text_args["horizontalAlignment"] = int(cell.findtext("horizontalAlignment", 0))
                    if cell.find("autoReturn") is not None:
                        text_args["autoReturn"] = int(cell.findtext("autoReturn", 0))
            ...
            txt = node.findtext("content", "") or node.text or ""
            params["text"] = txt
            # DEBUG: plain text draw
            print(f"[DEBUG] draw_text: '{txt}' @ x={params.get('x')} y={params.get('y')} size={params.get('fontSize')} rotation={params.get('rotation')}")
            api.draw_text(**params)
    ...
    if print_darkness is not None:
        commit_opts["darkness"] = print_darkness
    print(f"[DEBUG] commit_job opts: {commit_opts}")
    api.commit_job(**commit_opts)
    api.close_printer()
