...
class EmployeeController:
    ...
    def load_employees(self):
        """Build the full report‐tree under the current user."""
        report_tree: dict[int, list] = {}
    ...
    def on_search_text_changed(self, text: str):
        """
        Filter the displayed employees as the user types,
        showing only those who match beneath the current user.
        """
        t = text.strip().lower()

        # First, build the same full report_tree
        report_tree: dict[int, list] = {}
        to_visit = [self.current_user.user_id]
        while to_visit:
            sup_id = to_visit.pop()
        ...
        if not t:
            ...
            # Filtered: only include matching employees under root
            filtered = []
            for subs in report_tree.values():
                for emp in subs:
                    if (t in str(emp.user_id).lower()
                        or t in emp.last_name.lower()
                        or t in emp.first_name.lower()):
                        filtered.append(emp)

            # Build a single‐level tree: root → matching employees
            filtered_tree = {self.current_user.user_id: filtered}
            self.view.populate_tree(filtered_tree, self.current_user.user_id)
